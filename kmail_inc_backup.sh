...
for email in `cat /ESXi-NAS/kmail/accounts/userAccounts.txt`; do  echo "Started $email-- `date`";  /opt/zimbra/bin/zmmailbox -z -m $email getRestURL "//?fmt=tgz&query=date:$(date -d 'yesterday' '+%m/%d/%Y')" >  /ESXi-NAS/kmail/mailBoxes/$email-$(date -d 'yesterday' '+%Y%m%d').tgz ;   echo "$email Done-- `date`"; done
